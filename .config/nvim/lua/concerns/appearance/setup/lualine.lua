return {
  config = function()
    ...
    local function package_update_status()
      if require('pack.background').is_running() then
        return 'Pkg ↻'
      end
      return ''
    end
    ...
    local function build_vague_config()
      ...
      return {
        ...
        sections = {
          lualine_c = {
            { function() return '▊' end, color = mode_color, padding = { left = 0, right = 1 } },
          ...
          lualine_x = {
            { 'o:encoding', fmt = string.upper, cond = conditions.hide_in_width, color = { fg = colors.green, gui = 'bold' } },
            { 'fileformat', fmt = string.upper, icons_enabled = false, color = { fg = colors.green, gui = 'bold' } },
            { 'filetype', color = { fg = colors.cyan } },
            { package_update_status, color = { fg = colors.yellow, gui = 'bold' } },
            ...
            { function() return '▊' end, color = mode_color, padding = { left = 1 } },
          },
        },
      }
    end
    ...
    local function apply()
      ...
      local config = scheme == 'vague' and build_vague_config() or build_auto_config 'auto'
